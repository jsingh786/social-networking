$(document).ready(function() 
{
	...
	$("div#ilook_dt_wrapper span.buttons span#delete_records").click(function(){
		...
		{	
			...
			{
				confirmDeleteEnquiry(comma_sep.trim()); // delete jobs
			}
	...
	//Added by sjaiswal
    $( "div#dialog_confirm_delete_enquiry" ).dialog({
...
function listData( limit, offset, sort_column, sort_column_alias, sort_column_order, keep_old_sorting, filterText, countryFilterValue )
{
	...
	jQuery.ajax({
        url: "/" + PROJECT_NAME + "admin/enquiry/get-all-enquiry",
        type: "POST",
        dataType: "json",
        data: {
            	'limit':limit,
        ...
        success: function(jsonData) {
        	var tbody = '';
        	if( jsonData.enquiry_data.length > 0 )
        	{
            	var j;
            	//Customized part
	            for( i in jsonData.enquiry_data )
	            {
		            j = i;
		            if(j%2 == 0)
		            {
		        		tbody += '<tr role=" " class="odd">';
		            }
		            else
		            {
		        		tbody += '<tr role="row" class="even">';
		            }
		        	tbody += '<td><input type = "checkbox" name = "delete_cb[]" class = "delete_cb" onclick = "syncWithMasterCB()" value = "'+jsonData.enquiry_data[i].id+'"></td>';
		        	tbody += '<td id = "1">'+jsonData.enquiry_data[i].number+'</td>';
		        	tbody += '<td class = "2">'+jsonData.enquiry_data[i].type+'</td>';
		        	tbody += '<td class = "3">'+jsonData.enquiry_data[i].phone_number+'</td>';
		        	tbody += '<td class = "4">'+showCroppedText(jsonData.enquiry_data[i].email_id,30)+'</td>';
		        	tbody += '<td class = "5"><a title="view enquiry" class="reply" href="javascript:void(0);" onclick="viewEnquiry('+jsonData.enquiry_data[i].id+')">'+ showCroppedText(jsonData.enquiry_data[i].subject,25)+'</a></td>';
		        
		        	var desc = strip(jsonData.enquiry_data[i].description);
		        	var desc1 = showCroppedText(desc,20);
		        	
		        	tbody += '<td class = "6">'+desc1+'</td>';
		        	tbody += '<td class = "7" style = "text-align:center;">';
		        	...
		        	*/
		        	tbody += '<a title="Delete enquiry" class="delete" href="javascript:void(0);" onclick="confirmDeleteEnquiry('+jsonData.enquiry_data[i].id+')"></a>';
		        	//tbody += '<a title="reply enquiry" onclick="openEnquiryForm('+jsonData.enquiry_data[i].id+')" class="reply" href="/'+ PROJECT_NAME + 'admin/enquiry/reply/id/' +jsonData.enquiry_data[i].id+ '">Reply</a>';
		        	tbody += '<a title="reply enquiry" onclick="openEnquiryForm('+jsonData.enquiry_data[i].id+')" class="reply_enquiry" ></a>';
		        	tbody += '</td>';
		        	tbody += '</tr>';
	            }
        	...
        	if( jsonData.params.keep_old_sorting == 1 )
        	{
...
function syncWithMasterCB()
{
	 ...
	 * @author sjaiswal
	 */
	function confirmDeleteEnquiry(enquiry_ids)
	{
		$( "div#dialog_confirm_delete_enquiry" ).data('thiss',enquiry_ids).dialog( "open" );
	...
	function deleteEnquiry( ids )
	{
		...
		jQuery.ajax({
	        url: "/" + PROJECT_NAME + "admin/enquiry/delete-enquiry",
	        type: "POST",
	        dataType: "json",
	        data: {
	            	'ids_r': ids_r
	        ...
	        success: function(jsonData) {
		        if( jsonData == 1 )
		        {
			        ...
			        showDefaultMsg('Selected enquiry has been deleted.',1);
		        }
	        }
		});
	}
	...
	function removeattachment(filename)
	{
		...
		if( $("div#temp_uploads_info span").length > 0 )
		{
		    ...
		    $.ajax({
		        url : "/"+PROJECT_NAME+"admin/enquiry/discard-attachments",
		        data: {
			        	'files':filename
			        	},
		        type: "POST",
	...
	function viewEnquiry(enquiry_id)
	{
		
		//Bpopup for enquiry form.
			$("div#view_form").bPopup({
				...
				onOpen: function(){		
				    $.ajax({
				        url : "/"+PROJECT_NAME+"admin/enquiry/view",
				        data: {
				        ...
				        success: function(jsonData) {
					       // console.log(jsonData); 
				            if(jsonData)
				            {
				            	...
				            	html = '';
				            	html += '<div class="view-form-lt">';
				            	html += '<label>Phone Number</label>';
				            	html += '<span class="view_enquiry">'+jsonData.phone_number+'</span>' ;
				            	html += '<label>Email</label>';
				            	html += '<span class="view_enquiry">'+jsonData.email_id+'</span>' ;
				            	html += '<label>Subject</label>';
				            	html += '<span class="view_enquiry">'+jsonData.subject+'</span>' ;
				            	html += '<label>Description</label>';
				            	html += '<span class="desc">'+jsonData.description+'</span>' ;
				            	if(jsonData.attachment_data)
				            	{
					            	html += '<label>Attachment</label>';
					            	html += '<span class="view_enquiry">';
					            	for( i in jsonData.attachment_data )
						            {
						            	html += '<a href="'+PUBLIC_PATH+'/enquiry/enquiry_'+jsonData.id+'/'+jsonData.attachment_data[i].attachment_file+'" download="'+jsonData.attachment_data[i].actual_name+'">'+jsonData.attachment_data[i].attachment_file+'</a>' ;
						            	html += '</br>';
						            }
					            	html += '</span>';
				            	}
				            	...
				            	//html += jsonData.body;
				            	$('div.enquiry-pop-outer .view_content').html(html);
				            }
				        }
					});
