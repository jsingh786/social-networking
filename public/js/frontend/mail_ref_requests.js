$(document).ready(function(){
	...
	//perform search on enter 
	 $('#link_reqs_search').on('keypress', function(e){
		if(e.which == 13 )
			{  //Enter is key 13 
	...
	$("a#search_ref_reqs").click(function(){
		if($( "div.mail-hdr-search input[name=ref_reqs_search]" ).val().length <= 0 )
		{
...
function goBackToRefRequests()
{
	$( "div.mail-hdr-search input[name=ref_reqs_search]" ).val("");
	list_mails(1);
}
/**
 * lists reference request emails.
 * 
 * @author jisngh7
 * @version 1.0
 */
function list_mails(page)
{
	...
	jQuery.ajax({
        url: "/" + PROJECT_NAME + "mail/get-my-reference-requests",
        type: "POST",
        dataType: "json",
        data: { 'page' : page },
        timeout: 50000,
...
/**
 *  lists reference request emails according to the search word
 * 
 * @author hkaur5
 * @version 1.0
 */
function listSearchedMails( page, search_text )
{
	...
	jQuery.ajax({
        url: "/" + PROJECT_NAME + "mail/get-my-searched-reference-requests",
        type: "POST",
        dataType: "json",
        ...
        success: function(jsonData) {
        	...
        	row += '<div class="mail_select_all">';
        	row += '<input type="checkbox" value="" class="fl" name="mail_list_main_cb" id = "mail_list_main_cb">&nbsp; Select All';
        	row += '<span style = "float:right;"> <b>Search Results</b> <label style = "margin-right : 10px; cursor:pointer;" onclick = "goBackToRefRequests();">[Go Back To Reference Requests]</label></span>';
        	row += '</div>';
        	for( i in jsonData.records )
        	{
        		is_records_found = 1;
...
//			row += '<div class="bot">&nbsp;</div>';
			...
			row += '<ul>';
//			row += '<li><a href="#" class = "archive_ref" rel = "'+jsonData.msg_id+'"><span>Archive</span></a></li>';
        	...
        	$("div.mail-content-bot ul li a.archive_ref").click(function(){
        		var msg_id = $(this).attr("rel");
