$(document).ready(function(){
	...
	$('.pagination_nav').on('click','li.active', function()
	{
        ...
        if( $( "div.mail-hdr-search input[name=job_invitation_search]" ).val().length > 0 )
        {
        	listSearchedMails( page, $( "div.mail-hdr-search input[name=job_invitation_search]" ).val() );
        }
        else
        {	
        	list_mails(page);
	...
	//perform search on enter 
	 $('#job_invitation_search').on('keypress', function(e){
		if(e.which == 13 )
			{  //Enter is key 13 
			listSearchedMails( 1, $("div.mail-hdr-search input[name=job_invitation_search]").val() );
			}
	});
	 
	// Clicking on search button. 
	$("a#search_job_invitation").click(function(){
		if($( "div.mail-hdr-search input[name=job_invitation_search]" ).val().length <= 0 )
		{
...
function goBackJobInvitaions()
{
	$( "div.mail-hdr-search input[name=job_iinvitation_search]" ).val("");
	list_mails(1);
}
/**
 * lists inbox emails.
...
function list_mails(page)
{
	...
	jQuery.ajax({
        url: "/" + PROJECT_NAME + "mail/get-my-job-invitation-requests",
        type: "POST",
        dataType: "json",
        data: { 'page' : page },
        timeout: 50000,
...
/**
 * lists job invitaions inbox emails.
 * 
 * @author hkaur5
 * @version 1.0
 */
function listSearchedMails( page, search_text )
{
	...
	jQuery.ajax({
		url: "/" + PROJECT_NAME + "mail/get-my-searched-job-invitation-requests",
		type: "POST",
		dataType: "json",
		...
		success: function(jsonData) {
			...
			if( jsonData.records.length != 0 )
			{
				...
				row += '<div class="mail_select_all">';
				row += '<input type="checkbox" value="" class="fl" name="mail_list_main_cb" id = "mail_list_main_cb">&nbsp; Select All';
				row += '<span style = "float:right;"> <b>Search Results</b> <label style = "margin-right : 10px; cursor:pointer;" onclick = "goBackJobInvitaions();">[Go Back To Job Invitaions]</label></span>';
				row += '</div>';
				for( i in jsonData.records )
				{
					is_records_found = 1;
...
//			row += '<div class="bot">&nbsp;</div>';
			...
			row += '<ul>';
			row += '<li>';
//			row += '<a href="#" class = "archive_feedback" rel = "'+jsonData.msg_id+'">Archive</a>';
//			row += '<img src="'+IMAGE_PATH+'/folder-hover.png" width="16" height="13" align="absmiddle" />';
			row += '</li>';
...
//			row += '<img src="'+IMAGE_PATH+'/icon-trash-hover.png" width="14" height="16" align="absmiddle" />';
			row += '</li>';
			row += '</ul>';
			row += '</div>';
			row += '</div>';
