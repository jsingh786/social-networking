$(document).ready(function(){
	...
	$('.pagination_nav').on('click','li.active', function(){
        ...
        if( $( "div.mail-hdr-search input[name=trash_mails_search]" ).val().length > 0 )
        {
        	listSearchedMails( page, $( "div.mail-hdr-search input[name=trash_mails_search]" ).val() );
        }
        else
        {	
        	list_mails(page);
	...
	$("div#manage_pop_1 ul li a#multi_read").click(function(){
		...
		jQuery.ajax({
	        url: "/" + PROJECT_NAME + "mail/mark-read-trash-item",
	        type: "POST",
	        dataType: "json",
	...
	$("div#manage_pop_1 ul li a#multi_unread").click(function(){
		...
		jQuery.ajax({
			url: "/" + PROJECT_NAME + "mail/mark-unread-trash-item",
			type: "POST",
			dataType: "json",
			data: { 'msg_ids' : msg_ids },
			timeout: 50000,
	...
	$("div#manage_pop_1 ul li a#multi_inbox").click(function(){
		...
		jQuery.ajax({
	        url: "/" + PROJECT_NAME + "mail/move-trash-msg-to-inbox",
	        type: "POST",
	        dataType: "json",
	        data: { 'msg_ids' : msg_ids },
	        timeout: 50000,
	...
	//Clicking on multi-archive //by jsingh7
	$("div#manage_pop_1 ul li a#multi_trash_inbox").click(function(){
		$( "#dialog_confirm_multiple" ).data('thisss', $(this)).dialog( "open" );
	});
	...
	$( "#dialog_confirm" ).dialog({
	      ...
	      buttons: {
	       'Cancel': {
			...
			'Delete': function(){
				$(this).dialog( "close" );
				var iddd = addLoadingImage( $("#dialog_confirm").data('thiss'), "after");
				var msg_id = $("#dialog_confirm").data('thiss').attr("rel");
        		jQuery.ajax({
        			url: "/" + PROJECT_NAME + "mail/delete-trash",
        			type: "POST",
        			dataType: "json",
        			data: { 'msg_ids' : msg_id },
        			timeout: 50000,
	...
	$( "#dialog_confirm_multiple" ).dialog({
	      modal: true,
	      autoOpen: false,
	      width: 250,
	      show: {
	      ...
	      'Cancel': {
			...
			'Delete': function(){
				$(this).dialog( "close" );
				var iddd = addLoadingImage( $("#dialog_confirm_multiple").data('thisss'), "after");
				var msg_ids = "";
				var total = $("input[name='received_mail_ids[]']:checked:enabled",'div#mail_holder').length;
				$("input[name='received_mail_ids[]']:checked:enabled",'div#mail_holder').each(function( index ) {
				msg_ids += $(this).val();
	...
	$("#trash_mails_search").on('keypress',function(e){
		 //Enter is key 13 
		if(e.which == 13) 
			{
			listSearchedMails( 1, $("div.mail-hdr-search input[name=trash_mails_search]").val() );
			}
	});
	
	
	// Clicking on search button. 
	$("a#search_trash_items").click(function(){
		if($( "div.mail-hdr-search input[name=trash_mails_search]" ).val().length <= 0 )
		{
...
function goBackToTrashMails()
{
	$( "div.mail-hdr-search input[name=trash_mails_search]" ).val("");
	list_mails(1);
}
...
function list_mails(page)
{
	...
	jQuery.ajax({
        url: "/" + PROJECT_NAME + "mail/get-my-trash",
        type: "POST",
        dataType: "json",
        data: { 'page' : page },
        timeout: 50000,
	...
	//              <!-- Popup -->
	        		...
	        		row += '<li><a href="javascript:;" class = "delete_trash" rel = "'+jsonData.records[i].id+'">Delete</a></li>';
	        		row += '<li><a href="javascript:;" class = "inbox" rel = "'+jsonData.records[i].id+'">Move to inbox</a></li>';
	...
	//              <!-- col1 Ends -->
        	...
        	else
        	{
        		...
        		row += '<div class="no_messages-img"><img src="'+IMAGE_PATH+'/trash-empty.png"></div>';
        		row += '<div class="no_messages-data">Your Trash Folder is empty</div>';
        		row += '</div>';
        	}	
	        row = $(row);
        	...
        	$("div#mail_holder div.go_to_detail-text").click(function(){
        		show_trash_msg_detail($(this).attr("id"));
        	});
        	//end message detail
        	
        	//reply click in list.
        	$("div#mail_holder a.rply").click(function(){
        		show_trash_msg_detail($(this).attr("id"), 0, 1);
        	});
        	...
        	$("div#mail_holder a.forward").click(function(){
        		...
        		show_trash_msg_detail($(this).attr("id"), 1, 0);
        	...
        	$("div.manage-pop2 ul li a.delete_trash").click(function(){
        		$( "#dialog_confirm" ).data('thiss', $(this)).dialog( "open" );
        	});
...
function listSearchedMails( page, search_text )
{
	...
	jQuery.ajax({
        url: "/" + PROJECT_NAME + "mail/get-my-searched-trash",
        type: "POST",
        dataType: "json",
        ...
        success: function(jsonData) {
        	...
        	if( jsonData.records.length != 0 )
        	{
	        	...
	        	row += '<div class="mail_select_all">';
	        	row += '<input type="checkbox" value="" class="fl" name="mail_list_main_cb" id = "mail_list_main_cb">&nbsp; Select All';
	        	row += '<span style = "float:right;"> <b>Search Results</b> <label style = "margin-right : 10px; cursor:pointer;" onclick = "goBackToTrashMails();">[Go Back To Trash Mails]</label></span>';
	        	row += '</div>';
	        	for( i in jsonData.records )
	        	{
	//              <!-- col1 Starts -->
	        		...
	        		row += '<div class="go_to_detail-image mid" id = "'+jsonData.records[i].id+'" >';
	        		row += '<input name="received_mail_ids[]" class="fl mail_list_cb" type="checkbox" value="'+jsonData.records[i].id+'" />';
	        		row += '<div class ="mail-img-listing">';
		        	row += '<div class="go_to_detail-imagebox" style = "display:table-cell; width:50px; height:50px; vertical-align: middle; text-align: center;" user_id="'+jsonData.records[i].msg_sender_id+'">';
		        	row += '<img src="'+jsonData.records[i].prof_image+'"/>';
		        	...
		        	row += '</div>';
	        		row += '<div class="go_to_detail-text mid" id = "'+jsonData.records[i].id+'">';
	        		row += '<a id = "'+jsonData.records[i].id+'" class="text-purple2" href="javascript:;" style="font-size:14px;text-decoration: none;"><h4 class="font-arial" >'+jsonData.records[i].sender_firstname+' '+jsonData.records[i].sender_lastname+'</h4></a>';

	        		if(jsonData.records[i].subject == "" || jsonData.records[i].subject == null)
	        		{
		        		row += '<p><a id = "'+jsonData.records[i].id+'" class="font-arial text-purple2 text-purple-link" href="javascript:;" style="font-size:14px;"><i>No Subject</i></a></p>';
		        	}
	        		else
	        		{
		        		row += '<p><a id = "'+jsonData.records[i].id+'" class="font-arial text-purple2 text-purple-link" href="javascript:;" style="font-size:14px;">'+showCroppedText(jsonData.records[i].subject, 45)+'</a></p>';
		        	}
	        		row += '<p class="">';
	        		row += '<a id = "'+jsonData.records[i].id+'" class="text-purple-link text-purple2 font-weight-normal font-arial" href="javascript:;" style="font-size:14px;">';
	        		row += showCroppedText(jsonData.records[i].content, 150);
 ...
 * @version 1.1
 */
function show_trash_msg_detail( msg_id, forward_show, reply_show )
{
	...
	jQuery.ajax({
        ...
        success: function(jsonData) {
			...
			row += '</div>';
			row += '<div class="mid">';
			row += '<h4 class="mail-user-title">'+showCroppedText( jsonData.sender_firstname+' '+jsonData.sender_lastname, 35)+'</h4>';
			...
			row += '<input id="receiver_info" type="hidden" rel="'+jsonData.msg_receiver_id+'" value="">';
...
//			row += '<div class="bot">&nbsp;</div>';
			...
			row += '<ul>';
			row += '<li><a href="#" class = "inbox_sent" rel = "'+jsonData.msg_id+'">Inbox</a><img src="'+IMAGE_PATH+'/folder-hover.png" width="16" height="13" align="absmiddle" /></li>';
			...
			row += '<li><a href="javascript:;" class = "delete_trash" rel = "'+jsonData.msg_id+'">Delete</a><img src="'+IMAGE_PATH+'/icon-trash-hover.png" width="14" height="16" align="absmiddle" /></li>';
			row += '</ul>';
			row += '</div>';
...
//            row += '<label>TO</label>';
            ...
            row += '<a id="mail_contacts_popup" href="javascript:;"><img src="'+IMAGE_PATH+'/search-user-icon.png" style = "margin: 0 0 0 0px;" /></a>';
            row += ' </span>';
...
//            row += '<div class="mail-sendmsg-col2-msg">MESSAGE</div>';
            ...
            row += '<div class="fr"><a id="cancel" class="text-purple-link" href="javascript:;">Clear</a> </div>';
            row += '  </div>';
            row += ' </form>';
            row += '</div>';
...
//            row += '<label>TO</label>';
            ...
            row += '<a id="mail_contacts_popups" href="javascript:;"><img src="'+IMAGE_PATH+'/search-user-icon.png" style="margin:0 0 0 0px;"/></a>';
            row += ' </span>';
//            row += '<label>SUBJECT</label>';
        	if(jsonData.Subject == "" || jsonData.Subject == null){
...
//            row += '<div class="mail-sendmsg-col2-msg">MESSAGE</div>';
           ...
           // row += ' <textarea name="message_bodys" id="message_bodys" class="textarea" style="width:100%; border:none; min-height:170px; resize:none;" cols="" rows="">'+jsonData.contents+'</textarea>';
            row += ' </div>';
            row += ' <div class="mail-save-draft">';
            row += '<div class="fr"><a id="cancels" class="text-purple-link" href="javascript:;">Clear</a> </div>';
            row += '  </div>';
            row += ' </form>';
            row += '</div>';
          ...
          //Reply div end here
        	...
        	$("div.mail-content-bot ul li a.inbox_sent").click(function(){
        		var msg_id = $(this).attr("rel");
        	...
        	$(" div.mail-content-bot ul li a.delete_trash").click(function(){
        		var msg_id = $(this).attr("rel");
        		var sender_id = $("#sender_info").attr('rel');
        		var receiver_id = $("#receiver_info").attr('rel');
        		var iddd = addLoadingImage( $(this), "before");
        		jQuery.ajax({
        			url: "/" + PROJECT_NAME + "mail/delete-trash",
        			type: "POST",
        			dataType: "json",
        			data: { 'msg_ids' : msg_id ,'sender_id':sender_id,'receiver_id':receiver_id },
        			timeout: 50000,
        			success: function(jsonData) {
        				if( jsonData == 1 )
        				{	
			...
			//Forward attachment clear form data.
