...
//    					$('div.text div#'+jsonData+'.comment_text1').fadeIn();
	...
	$('select.edit_album_privacy').change(function(){
		if( $(this).val() == 4 )
		{
			$('input#custom_viewers_user_ids').val(); // delete values of custom viewers.
			__addOverlay();
	...
	// On click of ad button inside custom viewer popup store values 
	$("form#email_contacts_form #gmail_import_btn").click(function()
	{
		$('input#custom_viewers_user_ids').val();
		$("form#email_contacts_form #gmail_import_btn").hide();
		addLoadingImage( $(this), "before", 'loading_medium_purple.gif',61, 32 );
		var usersList = "";
		...
		$('input#custom_viewers_user_ids').val(usersList);
...
//				html=html+'</div>';			
				html=html+'</a>';
//				html=html+'<div class="photo_caption">';
//				if(jsonData.photo_desc[i])
//				{
//					html=html+'<div class="caption_hover col2-link2 text-white">'+showCroppedText(jsonData.photo_desc[i], 24);
//					html=html+'</div>';
//				}
//				else
//				{
//					html=html+'<div class="caption_hover col2-link2 text-white">Say some about this photo';
//					html=html+'</div>';
//				}
//				html=html+'</div>';
				if(jsonData.data[i].description)
				{
					html +='<textarea maxlength=255 rel ="'+jsonData.photoId[i]+'" class="edit_album_add_desc add_photo_desc" placeholder="Add description">'+jsonData.data[i].description+'</textarea>';
				}
				else
				{
					html +='<textarea maxlength=255  rel ="'+jsonData.photoId[i]+'" class="edit_album_add_desc add_photo_desc" placeholder="Add description"></textarea>';
				}
				html=html+'</div>';
			}
	...
	$.ajaxQueue({
		...
		success : function(jsonData) 
		{
			...
			for( i in jsonData.data)
        	{
				...
				if(jsonData.data[i].description)
				{
					html +='<textarea maxlength=255 rel ="'+jsonData.data[i].photo_id+'" class="edit_album_add_desc add_photo_desc" placeholder="Add description">'+jsonData.data[i].description+'</textarea>';
				}
				else
				{
					html +='<textarea maxlength=255 rel ="'+jsonData.data[i].photo_id+'" class="edit_album_add_desc add_photo_desc" placeholder="Add description"></textarea>';
				}
				html=html+'</div>';
	...
	jQuery.ajax({
		...
		success: function(jsonData) {
			...
			{	
	    		...
	    		comment += '</div>';
	    		comment += '<div onclick="hideComment(this);" style="text-align: right;" status="hide" class="hide_comment hide_comment_'+jsonData.comm_id+'"><img title="Hide comment" src="'+IMAGE_PATH+'/cross-grey.png"></div>';
	    		$("div#"+elemClass).html();
         	...
         	for( j in comments_json )
         	{
         		...
         		if( comments_json[j]['is_hidden'] == 0 )
     	    	{
    	    		...
    	    		html += '<span><a href = "/'+PROJECT_NAME+'profile/iprofile/id/'+comments_json[j]['comment_user_id']+'">'+comments_json[j]['comment_user_name']+'</a></span>';
     		    	...
     		    	{
     		    		html += '<div onclick="hideComment(this);" style="text-align: right;" class="hide_comment"><img title="Hide comment" src="'+IMAGE_PATH+'/cross-grey.png"></div>';
     		    	}
...
{
	var current_album_id= $(elem).attr('rel');
...
/**
 * Ajax call to save Album details
 * @author hkaur5
 * @param elem
 * 
 */
function saveAlbumInfo(elem)
{
	$(elem).hide();
	$loading_span_id = addLoadingImage( $(elem), "before", 'loading_small_purple.gif',44, 56 );

	if(!$('input#edit_album_name').val())
	{
		showDialogMsg('Oops!', "Album Title cannot be empty", 0,
				{
			    buttons: [
			        {
			            text: "OK",
		...
		$(elem).show();
		$("span#"+$loading_span_id).remove();
		return;
	}


	if($('input#edit_album_name').val().toLowerCase() =="default"
			|| $('input#edit_album_name').val().toLowerCase() =="cover photos"
			|| $('input#edit_album_name').val().toLowerCase() =="profile photos")
	{
		showDialogMsg('Oops!', 'You can not create an album with name as '+$('input#edit_album_name').val()+', Please choose some other name.', 0,
		{
			    buttons: [
			        {
			            text: "OK",
	...
	$.each( $("div#photos-container div.col2"), function( key, value ) {

		//console.log(key);
		//console.log($(this).attr("temp_path"));
		temp_photo_arr = {};
		var index  = $(this).children('textarea.edit_album_add_desc.add_photo_desc').attr('rel');
		
		temp_photo_arr[index] = $(this).children('textarea.edit_album_add_desc.add_photo_desc').val();
		photo_arr['index_'+key] = temp_photo_arr;
	});
...
//	return;
	...
	ajax_call = jQuery.ajax(
	{
	    url: "/" + PROJECT_NAME + "profile/update-album-details",
	    type: "POST",
	    dataType: "json",
	    data: { 'album_id':$(elem).attr('rel'), 
	    		'album_title':$('input#edit_album_name').val(),
	    		'privacy':$('select.edit_album_privacy').val(),
	    		'photo_id_desc_arr':JSON.stringify(photo_arr),
	    		'custom_viewers_comma_seperated':$('input#custom_viewers_user_ids').val()
	    	},
	    timeout: 50000,
	    success: function(jsonData) 
	    {
	    	if(jsonData)
    		{
	    		window.location.href="/" + PROJECT_NAME + "profile/photos/uid/"+$('input#albumOwnerId').val()+'/id/'+$(elem).attr('rel');
    		}
	    	else
    		{
	    		$(elem).show();
	    		$("span#"+$loading_span_id).remove();
	    		showDefaultMsg("Some thing went wrong while saving album changes! Please try again.", 2);
    		}
