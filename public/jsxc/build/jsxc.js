...
(function($) {
   ...
   jsxc.xmpp = {
      ...
      onRoster: function (iq) {
         ...
         if (customized_for_ilook == 1) {
            settingProfilePicsInRoster();
         }
   ...
   jsxc.gui = {
      ...
      tooltip: function (selector) {
         $(selector).tooltip({
            tooltipClass: "ilook-tooltip-styling",
            show: {
               delay: 600
            },
            content: function () {
      ...
      update: function (bid) {
         ...
         switch (data.msgstate) {
            case 0:
               ...
               if (AES_encryption_enabled == 1) {
                  we.find('.jsxc_transfer').removeClass('jsxc_enc jsxc_fin').attr('title', $.t('your_connection_is_encrypted'));
               }
               break;
            case 1:
            ...
            case 2:
               ...
               if (auto_start_otr == 1) {
                  ...
                  $('div.jsxc_window div.jsxc_transfer.jsxc_otr.jsxc_fin').click();
               }
   ...
   jsxc.gui.roster = {
      ...
      insert: function (bid, li) {
         console.log("Roster insertion process starts here.. ");
         var data = jsxc.storage.getUserItem('buddy', bid);
   ...
   jsxc.gui.window = {
      ...
      init: function (bid) {
         ...
         win.find('.jsxc_sendFile').click(function () {
            ...
            if (customized_for_ilook_file_transfer == 1) {
               openChatAttachmentForm(bid);

            } else {

               jsxc.gui.window.sendFile(bid,null);
            }
       ...
       * @param {boolean} args.sender
       * @param {integer} args.stamp
       * @param {string} args.extraClass [this class will be used in mesagae HTML]
       * @param {object} args.attachment Attached data
       * @param {string} args.attachment.name File name
       * @param {string} args.attachment.size File size
       * @param {string} args.attachment.type File type
      ...
      postMessage: function (message) {
         ...
         // remove html tags and re encode html tags
         ...
         if(customized_for_ilook_file_transfer == 1) {
            //Modifying UI of message.
            $("ul li[data-bid='" + message.bid + "'] div.jsxc_window div#"+message._uid.replace(':', '-')+".jsxc_chatmessage div:first-child")
                .append(attachmentPreview(message.msg));
         }
      ...
      restoreChat: function (bid) {
         var chat = jsxc.storage.getUserItem('chat', bid);
         var typing = jsxc.storage.getUserItem('Messaging', bid);
      ...
      sendFile: function (jid,fileLocation) {

         var bid = jsxc.jidToBid(jid);
         var win = jsxc.gui.window.get(bid);
         var res = Strophe.getResourceFromJid(jid);
         if (customized_for_ilook_file_transfer == 0) {
            if (!res) {
   ...
   jsxc.notification = {
      ...
      notify: function (title, msg, d, force, soundFile, loop, source) {
         ...
         var icon = o.icon || IMAGE_PATH + '/footer-logo-big.png';
   ...
   jsxc.gui.template['chatWindow'] = '<li class="jsxc_windowItem">\n' +
       ...
       '                  <ul>\n';
// if(customized_for_ilook == 0) {
//    jsxc.gui.template['chatWindow'] += '                     <li>\n' +
...
// }
   ...
   jsxc.gui.template['chatWindow'] += '                  </ul>\n' +
       ...
       '      </div>\n' +
       '      <div class="jsxc_fade">\n' +
// if(customized_for_file_transer == 0){'         <div class="jsxc_overlay">\n' +
// '            <div>\n' +
// '               <div class="jsxc_body" />\n' +
